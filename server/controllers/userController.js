...
const paymentRazorpay = async (req, res) => {
    try {
        ...
        //we need userId and planId for razor payment
        const { userId, planId } = req.body

        // find the user using userId
        const userData = await userModel.findById(userId)

        // checking for planId and userdata available or not
        if (!userData || !planId) {
            return res.json({ success: false, message: 'Missing Details' })
        }
        ...
        // Creating razorpay Order// options is used to create the orders using razorpay instance
        await razorpayInstance.orders.create(options, (error, order) => {
            //if any error occurs while creating the order then we will get
            //the error in the parameter and if the order created which is
            //in parameter the order will be created 
            if (error) {
                console.log(error);
...
const verifyRazorpay = async (req, res) => {
    try {
        ...
        if (orderInfo.status === 'paid') {
            ...
            // if the transaction payment is already true it means it is already verfied
